...
const getPositions = (props, ref) => {
  ...
  const orientationX =
    props.targetCoordinates.x > overlayBounds.width / 2 ? 'left' : 'right'
  const quotientY = Math.floor(
    props.targetCoordinates.y / (overlayBounds.height / 3)
  )
  const orientationY =
    quotientY < 1 ? 'top' : quotientY < 2 ? 'middle' : 'bottom'
  ...
  if (orientationX === 'left') {
    modalStyle.right =
      overlayBounds.width - (props.targetCoordinates.x + offset.modalX)
    ...
    arrow: (
      <img
        ...
        className={classNames(
          styles.arrow,
          styles[`orientation-${orientationX}`],
          styles[`orientation-${orientationY}`]
        )}
        style={arrowStyle}
      />
    ),
