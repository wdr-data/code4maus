...
type LibraryItem = LibrarySprite | LibraryCostume | LibraryBackdrop | LibrarySound
type AssetTypeMap = {
    "name": LibraryItem,
...
const dedupe = <T extends LibraryItem>(
    ...
    return [ ...library, ...Array.from(inserts.values()) ]
        .sort((a, b) =>
...
const makeImageAsset = <T extends LibraryImageAssetType>(
    type: T,
) => (
    target: SB3TargetMap[typeof type],
): LibraryImageAsset<T>[] => target.costumes.map(costume => ({
    name: costume.name,
    md5: costume.md5ext,
    type,
    tags: [],
...
const makeSounds = (
    target: SB3Target,
): LibrarySound[] => target.sounds.map(sound => ({
    name: sound.name,
    md5: sound.md5ext,
    sampleCount: sound.sampleCount,
    rate: sound.rate,
...
const reduceToMap = <T extends LibraryItem>(acc: Map<string, T>, item: T): Map<string, T> => {
...
const importMain = async () => {
    ...
    const tasks: ImportType[] = (type === "all")
        ? types
        : type.split(',')
    ...
    return Promise.all(tasks.map(async (type) => {
