...
const getLibraryContent = async () => {
    const sprites = JSON.parse(
        fs.readFileSync(path.resolve(__dirname, '../../src/lib/libraries/sprites.json'), 'utf-8')
    );
    ...
    const singles = [ 'backdrops', 'costumes', 'sounds' ].map(
        (type) => JSON.parse(
            fs.readFileSync(path.resolve(__dirname, `../../src/lib/libraries/${type}.json`), 'utf-8')
        ).map((a) => a.md5),
    );
...
const getProjectContent = function() {
    ...
    return globby(
        ...
        .then((projectPaths) => Promise.all(projectPaths.map(
...
function getAllAssets() {
    return Promise.all([ getProjectContent(), getLibraryContent() ])
        .then((results) => results.reduce((prev, current) => prev.concat(current), []))
